...
in der folgendem Abbildung blau dargestellt, die Regressionsgerade rot.
...
Paramter so bestimmen, dass diese Abweichungen insgesamt möglichst klein werden?
...
Für die lineare Regression wird häufig als Fehlermaß die **Methode der kleinsten
Quadrate** verwendet (siehe [Wikipedia → Methode der kleinsten
...
Danach summieren wir die Residuen auf. Dabei kann es passieren, dass sich
positive und negative Abweichungen gegenseitig aufheben. Liegt ein Messpunkt
beispielsweise unterhalb der Geraden, ist das Residuum negativ. Liegt ein
anderer Messpunkt oberhalb der Geraden, ist das Residuum positiv.
...
beiden Parameter $w_0$ und $w_1$, also den y-Achsenabschnitt $w_0$ in
`.intercept_` und die Steigung $w_1$ in `.coef_`.
...
Mit der trainierten linearen Funktion $y =  2179 + 85.2 \cdot x$ könnten wir nun
