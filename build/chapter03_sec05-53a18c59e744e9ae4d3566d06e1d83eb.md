...
In dieser Woche haben wir Series-Objekte, statistische Kennzahlen und
Boxplots am Beispiel von Wellendurchmessern aus der Fertigung kennengelernt.
Die folgenden Übungen trainieren dieselben Werkzeuge an fachfremden
Datensätzen. Das ist kein Zufall: Die Werkzeuge Series, `.describe()` und
`px.box()` funktionieren für jede Tabelle mit Zahlen, egal ob es sich um
Wellendurchmesser, Studierendenzahlen oder Gebrauchtwagenpreise handelt.

```{admonition} Hinweis zum EDA-Bericht
:class: note
In Woche 6 erstellen Sie in Ihrem Projektteam einen eigenen EDA-Bericht
(Abgabe 1). Genau die Arbeitsschritte dieser Übungen, Series erzeugen,
Kennzahlen mit `.describe()` ermitteln, Boxplots interpretieren, erwartet das
Kriterienraster dieser Abgabe. Es lohnt sich, diese Übungen vor Woche 6
...
```{admonition} Übung a)
:class: tip
Speichern Sie die Daten zu den Studentinnen an Fachhochschulen als
Pandas-Series. Verschaffen Sie sich einen Überblick über die statistischen
Kennzahlen. Lesen Sie dann ab: In welchem Bundesland studieren die wenigsten
Studentinnen und in welchem Bundesland die meisten?
```
...
Die minimale Anzahl an Studentinnen ist 5767, die maximale Anzahl 106755. Mit
`print` lassen wir den kompletten Datensatz anzeigen:

```python
print(stud_fh_weiblich)
```

Wir lesen ab: am wenigsten Studentinnen an Fachhochschulen studieren im
Saarland und am meisten in Nordrhein-Westfalen.
````
...
Erstellen Sie ein Series-Objekt für diesen Datensatz und überprüfen Sie, ob
auch dort das Saarland das Minimum und Nordrhein-Westfalen das Maximum hat.
Lassen Sie Minimum und Maximum mit `.min()` und `.max()` ausgeben und
kontrollieren Sie durch Anzeige des Datensatzes, welches Bundesland dazugehört.
...
Ergebnis: Minimum im Saarland (7806) und Maximum in Nordrhein-Westfalen
(235564).
...
Bundeslandes ab. Das Saarland ist das kleinste Flächenland,
Nordrhein-Westfalen das bevölkerungsreichste Bundesland Deutschlands.
````
...
Fachhochschulen. Achten Sie darauf, einen anderen Variablennamen für das
Diagramm zu verwenden (z. B. `diagramm2` statt `diagramm`), damit der erste
Boxplot nicht überschrieben wird.
...
Fachhochschulen". Das betroffene Bundesland ist Nordrhein-Westfalen mit
132.976 Studenten. Bei den Studentinnen an Fachhochschulen gibt es keinen
Ausreißer.
````
...
Hinweis: Die Boxplots müssen nicht einzeln angezeigt werden, speichern Sie sie
aber in den Variablen `fig1`, `fig2`, `fig3` und `fig4`, damit Sie sie in der
nächsten Aufgabe vergleichen können.
```
...
1. An welcher Hochschulart (Uni oder FH) ist die Streuung der
   Studierendenzahlen größer?
...
1. *Streuung Uni vs. FH:* Die Streuung ist an Universitäten deutlich größer.
   Der Interquartilabstand (Q3 - Q1) beträgt bei Unis ca. 48000 (Männer) bzw.
   54000 (Frauen), bei FH nur ca. 32000 (Männer) bzw. 43000 (Frauen). Das
   liegt vermutlich daran, dass es in großen Bundesländern mehr Universitäten
   gibt.
...
## Zusatzübung: Gebrauchtwagenpreise

```{admonition} Übung f)
:class: tip
Gegeben sind die Verkaufspreise (in Euro) von zehn Autos der Marken Audi, BMW
und Citroen, die auf der Plattform Autoscout24 angeboten wurden:
...
1. Erzeugen Sie ein Series-Objekt mit den Automarken/-nummern als Index und
   dem Namen 'Gebrauchtwagen'.
2. Verschaffen Sie sich mit `.describe()` einen Überblick über die
   statistischen Kennzahlen.
3. Erstellen Sie einen beschrifteten Boxplot mit Titel 'Gebrauchtwagen-Analyse'
   und y-Achsenbeschriftung 'Preis [EUR]', der alle Datenpunkte zeigt.
4. Welche zwei Autos liegen preislich am weitesten auseinander?
```
...
preise = pd.Series(preisliste, index=autos, name='Gebrauchtwagen')
preise.describe()
...
Audi Nr. 1 (1.999 EUR, günstigstes) und BMW Nr. 1 (46.830 EUR, teuerstes)
liegen preislich am weitesten auseinander.
````
