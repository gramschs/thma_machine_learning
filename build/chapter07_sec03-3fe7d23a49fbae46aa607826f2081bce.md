...
```{code-cell} python
v_vergleich = np.linspace(v_min, 180, 200)
...
y_prognose = model.predict(X_vergleich)
...
```python
for grad in [3, 8]:
    transformator = PolynomialFeatures(degree=grad)
    X = transformator.fit_transform(daten[['Geschwindigkeit [km/h]']])
    y = daten['Bremsweg [m]']
    ...
    r2 = model.score(X, y)
    X_vergleich = transformator.transform(vergleichsdaten[['Geschwindigkeit [km/h]']])
    r2_vergleich = modell.score(X_vergleich, y_vergleich)
    print(f'Grad {grad}: R2 Training {r2:.4f}, R2 Vergleich {r2_vergleich:.4f}')
```
