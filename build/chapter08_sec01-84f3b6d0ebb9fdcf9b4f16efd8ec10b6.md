...
1. Lesen Sie die Datei ein und verwenden Sie die Spalte `Nummer` als
   Zeilenindex.
2. Verschaffen Sie sich mit `info()` einen Überblick. Bei welchen Merkmalen
   fehlen Werte?
3. Bestimmen Sie mit `isnull()` und `.sum()` die Anzahl der fehlenden Werte pro
   Spalte.
4. Lassen Sie sich alle Druckaufträge anzeigen, bei denen die `Druckzeit (min)`
   fehlt.
```

```{code-cell} python
# Diese Code-Zelle können Sie für Ihre Lösung nutzen.
```
...
```{admonition} Mini-Übung
:class: tip
Arbeiten Sie mit dem DataFrame `druckversuche` aus der vorherigen Mini-Übung
weiter.

1. Das Merkmal `Oberflaechenguete` hat viele Lücken und lässt sich ohnehin aus
   der `Schichthoehe (mm)` ableiten. Löschen Sie die Spalte und speichern Sie
   das Ergebnis in `druckversuche_bereinigt`.
...
3. Sehen Sie sich diesen Datenpunkt genau an. Welche zwei Einträge sind
   auffällig?
4. Lassen Sie sich mit `unique()` die Werte der Spalte `Drucktemperatur (C)`
   ausgeben. Warum wurde diese Spalte beim Import als `object` und nicht als
   Zahl eingelesen?
5. Löschen Sie den auffälligen Druckauftrag aus `druckversuche_bereinigt`.
```
...
```{admonition} Mini-Übung
:class: tip
Arbeiten Sie mit `druckversuche_bereinigt` aus der vorherigen Mini-Übung weiter.

1. Ersetzen Sie die fehlenden Werte der Spalte `Farbe` durch den Eintrag
   `'keine Angabe'`.
2. Die Spalte `Druckzeit (min)` ist numerisch und stark rechtsschief verteilt,
   es gibt also einige wenige sehr lange Drucke. Ersetzen Sie die fehlenden
   Werte durch den **Median** der vorhandenen Werte.
3. Kontrollieren Sie mit `isnull().sum()`, dass keine fehlenden Werte mehr übrig
   sind.
4. Begründen Sie in einem Satz, warum hier der Median dem Mittelwert vorzuziehen
   ist.
```
