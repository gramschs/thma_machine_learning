...
Mittelwert (engl. Mean) und Standardabweichung (engl. Standard Deviation) werden
mit `.mean()`und `.std()` berechnet. Minimum und Maximum erhalten wir über die
Methoden `.min()` und `.max()`.
