...
Das Ergebnis ist eine Pandas-Series. Die Namen der Kennzahlen wie `'mean'`,
`'std'`, `'min'` und `'max'` bilden dabei den Index dieser Series. Deshalb
können wir auf einzelne Kennzahlen genauso zugreifen wie auf andere Elemente
einer Series, also über den Index in eckigen Klammern.
