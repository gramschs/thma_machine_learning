...
* [ ] Sie wissen, was die Angaben `samples` und `value` bei der Visualisierung
  des Entscheidungsbaumes bedeuten.
* [ ] Sie wissen, was das **Gini-Impurity-Kriterium** ist.
* [ ] Sie kennen weitere Parameter für Entscheidungsbäume wie `random_state=`
  oder `criterion=`.
```
...
entsteht. In einem echten Projekt würde man diesen Parameter nicht dazu nutzen,
um ein Wunschergebnis zu erzwingen.
...
plot_tree(modell_druck,
    ...
    class_names=['nicht erfolgreich', 'erfolgreich']);
```
3. Die Baumstruktur ist identisch zur ersten Mini-Übung, da sich die Daten
   eindeutig trennen lassen. Unterschiedlich ist nur die angezeigte
   Kennzahl: Statt `gini = 0.5` steht im Wurzelknoten jetzt
   `entropy = 1.0`, dem für zwei gleich große Klassen maximalen
   Entropie-Wert.
````
