...
die Beschriftung der x- und y-Achse. Die automatisch erzeugte Legende zeigt
hier nur einen einzigen, redundanten Eintrag und liefert keinen zusätzlichen
...
fig = px.histogram(data['Kilometerstand (km)'], nbins=10, 
    title='Künstlich gewählte Intervalle bzgl. des Kilometerstandes (km)')
fig.update_layout(
    xaxis_title='Intervalle der Kilometerstände (km)',
    yaxis_title='Anzahl Autos',
    legend_title='Anzahl Autos pro Intervall',
)
fig.show()
```
...
gewählt, bleiben ggf. einige Bins leer.
...
fig.update_layout(
    ...
    yaxis_title='Anzahl Druckversuche',
    legend_title='Anzahl Druckversuche pro Intervall',
)
fig.show()
```
