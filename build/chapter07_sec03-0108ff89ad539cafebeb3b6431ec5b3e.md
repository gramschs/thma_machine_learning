...
r2 = model.score(X, y)
print(f'R2-score Trainingsdaten: {r2:.4f}')
```
...
r2_vergleich = model.score(X_vergleich, y_vergleich)
...
y_vergleich = vergleichsdaten['Bremsweg [m]']
r2_score = model.score(X_vergleich, y_vergleich)
print(f'R2-score im Vergleich zur Faustformel: {r2_score:.4f}')
...
1. Welches Modell passt am besten zur Faustformel (Spalte R² Vergleich)?
2. Beim Polynom Grad 14 ist der R²-Score im Vergleich zur Faustformel stark
   negativ. Was bedeutet ein negativer R²-Score (siehe Kapitel 7.1)?
...
:class: tip
:class: dropdown
1. Grad 2 (quadratisch) passt mit R² ≈ 0.9999 am besten zur Faustformel.
2. Ein negativer R²-Score bedeutet, dass das Modell schlechter ist als die
   einfache Vorhersage des Mittelwerts. Beim Polynom Grad 14 liegt das vor allem
   an den Geschwindigkeiten oberhalb von 150 km/h: Dort verlässt das Modell den
   trainierten Bereich und liefert völlig überzogene Bremswege, die den R²-Score
   stark nach unten ziehen.
3. Underfitting - gute Passung - Overfitting:
    * Grad 1 (linear) - Underfitting (zu einfaches Modell, systematische
      Abweichungen im ganzen Bereich)
    * Grad 2 (quadratisch) - gut angepasst (beide Scores nahe 1.0, passt zur
      wahren Faustformel)
    * Grad 14 - Overfitting (sehr flexibel, wird dadurch unruhig und instabil,
      außerhalb der Trainingsdaten unbrauchbar)
4. Die Visualisierung zeigt schon oberhalb von 150 km/h, dass die Prognose steil
   ansteigt. Bei 200 km/h prognostiziert das Modell einen Bremsweg von mehreren
   hundert Kilometern, also einen völlig unrealistischen Wert.
```
