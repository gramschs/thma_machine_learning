...
Spaltennamen. Das neu erzeugte Series-Objekt kann also beispielsweise mit
...
Zusammengefasst besteht ein DataFrame aus Series-Objekten.
...
Tabelle eine einzelne Zeile auswählen. Dazu gibt es den Indexer `.loc`. Danach
werden wieder eckige Klammern benutzt, wobei diesmal der Zeilenindex und nicht
der Spaltenname verwendet wird.
...
indem ein neuer Zeilenindex vergeben wird.

```{admonition} Warnung
:class: warning
Das Einfügen einer neuen Spalte oder Zeile über einen neuen Index funktioniert
nur, wenn die neuen Daten das richtige Format haben. Im Zweifelsfall sollte die
