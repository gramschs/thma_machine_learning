...
Alle drei Architekturen erreichen auf den Testdaten denselben Score wie das
Netz mit `hidden_layer_sizes=(5, 5)` weiter unten. Bei `(15, 10, 5)` ist der
Trainingsscore aber perfekt, während der Testscore nicht besser wird: Das Netz
hat die Trainingsdaten auswendig gelernt statt allgemeine Muster zu erlernen,
also Overfitting. Ein größeres oder tieferes Netz bringt hier keinen Vorteil.
Wir wählen deshalb die einfachste Architektur, die gute Ergebnisse liefert.
```
