...
handelt. Für die Einordnung ist jedoch die fachliche Bedeutung des Merkmals
entscheidend.
...
Marken. Allgemein gesagt, gibt es für das Merkmal »Marke« 41 Kategorien. Ein
nicht-numerisches Merkmal, das nur eine begrenzte Anzahl von Werten annehmen
kann, wird als kategoriale Variable bezeichnet. Ihre konkreten Werte werden als
kategoriale Daten oder qualitative Daten bezeichnet.
...
Merkmale des Auto-Datensatzes sind kategorial? In der Praxis spricht man von
kategorialen Daten, wenn die Anzahl der Kategorien deutlich kleiner als die
Anzahl der Datenpunkte ist (oft als Faustregel: < 5-10 % der Beobachtungen).

```{admonition} Mini-Übung
:class: tip
Suchen Sie sich drei nicht-numerische Merkmale aus und bestimmen Sie
die Anzahl der einzigartigen Einträge dieses Merkmals.
...
print(f'Die Spalte Modell hat {anzahl_einzigartige_werte} einzigartige Werte.')
```

Offensichtlich gibt es 561 einzigartige Werte für die Spalte Modell.
...
```{code}
Die Spalte Marke hat 41 einzigartige Werte.
Die Spalte Modell hat 561 einzigartige Werte.
Die Spalte Farbe hat 15 einzigartige Werte.
Die Spalte Erstzulassung hat 12 einzigartige Werte.
Die Spalte Jahr hat 1 einzigartige Werte.
Die Spalte Preis (Euro) hat 4926 einzigartige Werte.
Die Spalte Leistung (kW) hat 260 einzigartige Werte.
Die Spalte Leistung (PS) hat 260 einzigartige Werte.
Die Spalte Getriebe hat 4 einzigartige Werte.
Die Spalte Kraftstoff hat 10 einzigartige Werte.
Die Spalte Verbrauch (l/100 km) hat 238 einzigartige Werte.
Die Spalte Verbrauch (g/km) hat 616 einzigartige Werte.
Die Spalte Kilometerstand (km) hat 10730 einzigartige Werte.
Die Spalte Bemerkungen hat 16547 einzigartige Werte.
```
````
...
Merkmal Bemerkungen 16547 verschiedene Werte. Die Definition von kategorialen
bzw. qualitativen Daten besagt aber, dass nur eine begrenzte Anzahl von
verschiedenen Werten angenommen wird. Das ist hier nicht mehr der Fall, so dass
wir das Merkmal Bemerkung nicht als kategoriales/qualitatives Merkmal einstufen.
Die kategorialen Merkmale in dem Beispiel sind also Marke, Modell, Farbe,
