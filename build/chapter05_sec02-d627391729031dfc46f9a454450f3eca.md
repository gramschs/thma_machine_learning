...
Möchten wir beispielsweise nur Autos der Marken Audi, BMW oder Mercedes-Benz
auswählen, müssten wir mit den bisherigen Vergleichsoperatoren drei Bedingungen
einzeln mit `|` (oder) verknüpfen. Mit `.isin()` geht das kürzer: Wir übergeben
einfach eine Liste der gesuchten Marken.

```{code-cell}
bedingung_marke = data['Marke'].isin(['audi', 'bmw', 'mercedes-benz'])
print(f"Anzahl Autos: {bedingung_marke.sum()}")
```
