...
Der Zusatz `<NumpyExtensionArray>` in der Ausgabe ist dabei nur eine
Formatierung von Pandas und keine neue Datenstruktur. Dahinter stecken weiterhin
einfach nur die Verkaufspreise.
...
```{code-cell} ipython3
preise.name = 'Verkaufspreis in Euro'
print(preise)
```
...
```{code-cell} ipython3
autos = ['Audi Nr. 1', 'Audi Nr. 2', 'Audi Nr. 3', 'BMW Nr. 1', 'BMW Nr. 2', 'Citroen Nr. 1', 'Citroen Nr. 2', 'Citroen Nr. 3', 'Citroen Nr. 4', 'Citroen Nr. 5']
preise = pd.Series(preisliste, index=autos, name='Verkaufspreis in Euro')
print(preise)
```
