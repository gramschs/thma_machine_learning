...
for grad in [3, 8]:
    ...
    r2_vergleich = model.score(X_vergleich, y_vergleich)
    print(f'Grad {grad}: R2 Training {r2_train:.4f}, R2 Vergleich {r2_vergleich:.4f}')
```
...
3. Der Trainings-Score ist bei Grad 2, 3 und 8 identisch (0.9979). Mehr
   Freiheitsgrade verbessern die Anpassung an die Trainingsdaten also nicht.
4. Grad 3 passt mit R² ≈ 0.9994 fast perfekt zur Faustformel, Grad 8 nur noch mit
   R² ≈ 0.55. Obwohl beide Modelle die Trainingsdaten gleich gut beschreiben,
   verallgemeinert Grad 8 deutlich schlechter, hier zeigt sich beginnendes
   Overfitting. Für die Prognose wählt man das einfachste Modell, das die Daten
   gut beschreibt, hier also das quadratische Modell mit Grad 2.
````
