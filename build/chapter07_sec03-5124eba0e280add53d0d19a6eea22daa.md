...
Wir erzeugen das PolynomialFeatures-Objekt mit der Option `degree=2`, um die
...
Bei hohem Polynomgrad werden die Merkmale allerdings sehr groß: $150^{14}$ ist
eine Zahl mit über 30 Stellen. Damit die Berechnung numerisch zuverlässig
bleibt, skalieren wir die Geschwindigkeit vorher mit dem `MinMaxScaler` auf den
Bereich 0 bis 1. Wie bei `PolynomialFeatures` wird der Skalierer mit `fit` an die
Trainingsdaten angepasst und mit `transform` auf neue Daten angewendet.

```{code-cell} python
from sklearn.preprocessing import MinMaxScaler

# Geschwindigkeit skalieren, dann Merkmale erzeugen
skalierer = MinMaxScaler()
v_skaliert = skalierer.fit_transform(daten[['Geschwindigkeit [km/h]']])

polynom_transformator = PolynomialFeatures(degree = 14)
X = polynom_transformator.fit_transform(v_skaliert)
y = daten['Bremsweg [m]']
...
X_vergleich = polynom_transformator.transform(
    skalierer.transform(vergleichsdaten[['Geschwindigkeit [km/h]']]))
y_vergleich = vergleichsdaten['Bremsweg [m]']
r2_vergleich = modell.score(X_vergleich, y_vergleich)
print(f'R2-score im Vergleich zur Faustformel: {r2_vergleich:.4f}')
...
Im mittleren Geschwindigkeitsbereich schmiegt sich die Prognosekurve eng an die
einzelnen Datenpunkte, enger als beim quadratischen Modell. Der Trainings-R² ist
mit 0.9989 dann auch etwas höher als beim quadratischen Modell (0.9979): Ein
Polynom 14. Grades enthält das quadratische als Spezialfall und kann die
Trainingsdaten daher mindestens so gut anpassen.

An den Rändern wird das Modell aber unzuverlässig. Bei 30 km/h sagt es sogar
einen negativen Bremsweg voraus, und oberhalb von 150 km/h, also außerhalb der
Trainingsdaten, verlässt die Prognose steil den dargestellten Bereich der
y-Achse. Der R²-Score im Vergleich zur Faustformel bezieht auch diese
Geschwindigkeiten über 150 km/h ein und wird dadurch stark negativ. Das Polynom
14. Grades hat sich zu stark an die einzelnen Trainingspunkte angepasst und
beschreibt den zugrundeliegenden Zusammenhang schlechter. Es liegt
**Overfitting** vor.
...
Modellprognose zur wahren Faustformel im Bereich bis 180 km/h passt, also
einschließlich der Geschwindigkeiten außerhalb der Trainingsdaten.

| Polynomgrad | R² (Trainingsdaten) | R² (Vergleich Faustformel) |
| --- | --- | --- |
| 1 | 0.9641 | 0.9311 |
| 2 | 0.9979 | 0.9999 |
| 14 | 0.9989 | -840774.6994 |
...
1. Trainieren Sie ein polynomiales Regressionsmodell mit Grad 3 und eines mit
   Grad 8. Gehen Sie genauso vor wie oben: Geschwindigkeit mit dem `MinMaxScaler`
   skalieren, Merkmale mit `PolynomialFeatures` erzeugen, `LinearRegression`
   trainieren, dann den R²-Score auf den Trainingsdaten und den R²-Score im
   Vergleich zur Faustformel berechnen. Verwenden Sie dazu die schon vorhandenen
   `vergleichsdaten`.
2. Notieren Sie die beiden zusätzlichen Zeilen (Grad 3 und Grad 8) für die
   Tabelle oben.
3. Wie verändert sich der Trainings-Score von Grad 2 über Grad 3 zu Grad 8?
...
**Zu 3.** Der Trainings-Score steigt mit dem Polynomgrad leicht an (0.9979,
0.9979, 0.9986). Ein Polynom höheren Grades enthält die niedrigeren Grade als
Spezialfall und kann die Trainingsdaten daher mindestens so gut anpassen.

**Zu 4.** Grad 3 trifft die Faustformel im ganzen betrachteten Bereich sehr gut
(R² ≈ 0.9994). Bei Grad 8 wird der Vergleichs-Score negativ, weil das Modell
oberhalb von 150 km/h, also außerhalb der Trainingsdaten, unbrauchbar wird.
Obwohl Grad 8 die Trainingsdaten minimal besser anpasst, beschreibt es den
wahren Zusammenhang schlechter. Für die Prognose wählt man das einfachste
Modell, das die Daten gut beschreibt, hier also das quadratische Modell mit
Grad 2.
````

Der Trainings-Score steigt mit dem Polynomgrad an: Ein Polynom höheren Grades
enthält die niedrigeren Grade als Spezialfall und kann die Trainingsdaten daher
mindestens so gut anpassen. Bei Grad 14 ist der Trainings-Score mit 0.9989
minimal höher als beim quadratischen Modell (0.9979). Die zusätzliche
Flexibilität hilft aber nicht: Das Modell beschreibt die wahre Faustformel
schlechter und wird außerhalb des trainierten Bereichs völlig unbrauchbar,
erkennbar am stark negativen R²-Score im Vergleich zur Faustformel. Beim
quadratischen Modell sind beide Scores nahezu perfekt und passen zur Faustformel.
Daher wählen wir für das ML-Modell ein quadratisches Regressionsmodell.
...
Hyperparameters (Polynomgrad) wichtig: Ein zu niedriger Grad führt zu
Underfitting, ein zu hoher Grad zu Overfitting. Im nächsten Kapitel werden wir
uns ansehen, wie Hyperparameter systematisch gewählt werden.
