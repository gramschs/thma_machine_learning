...
Technik heißt Slicing (englisch für "in Scheiben schneiden") und funktioniert
mit einem Doppelpunkt zwischen Start und Ende. Wählen wir alle Autos von
`Audi Nr. 1` bis `BMW Nr. 1` aus:
...
Wir erhalten vier Autos, denn `BMW Nr. 1` ist in der Auswahl **enthalten**.
Versuchen wir nun, denselben Teilbereich über die Positionen zu erhalten. Die
vier Autos stehen an den Positionen 0, 1, 2 und 3:
...
:class: warning
Beim Slicing mit `.loc[]` ist die Obergrenze **eingeschlossen**, beim Slicing
mit `.iloc[]` ist sie **ausgeschlossen** (wie bei Python-Listen). Der Ausdruck
`preise.loc['Audi Nr. 1':'BMW Nr. 1']` liefert vier Elemente inklusive
`BMW Nr. 1`, der Ausdruck `preise.iloc[0:4]` liefert die Positionen 0 bis 3.
```
...
Die folgende Aufgabe bearbeiten Sie zu zweit im Studio-Format.
