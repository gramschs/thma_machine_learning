...
Im Series-Objekt sind 18 566 Einträge vom Datentyp `bool` gespeichert. Das
...
Bereits bei den ersten 10 Autos im gefilterten Datensatz sind auffällig viele
weiße Autos. Es könnte sich hierbei um Taxis handeln, die häufig weiß lackiert
sind. Diese erste Vermutung müssten wir jedoch noch weiter untersuchen, indem
wir beispielsweise den relativen Anteil der weißen Autos in dem gefilterten
Datensatz mit dem relativen Anteil im Gesamtdatensatz vergleichen. Aber nun
legen wir unser Augenmerk auf das Gruppieren von Daten.
...
nach den unterschiedlichen Werten einer Spalte zu gruppieren:
...
| Frage | Technik | Beispiel |
| --- | --- | --- |
| Wie viele Audis gibt es? | Filtern | `data['Marke'] == 'audi'` |
| Durchschnittspreis je Marke? | Gruppieren | `.groupby('Marke').mean()` |
...
einzigartigen Werte einer Spalte zu gruppieren, dann ist `.groupby()` in
Kombination mit einer Aggregationsmethode wie `.mean()` oft der effizienteste
Weg. Damit haben wir ein wichtiges Werkzeug kennengelernt, um Datensätze gezielt
einzugrenzen und zu vergleichen. Im nächsten Kapitel wenden wir uns der
Visualisierung kategorialer Daten zu und lernen, wie sich Barplots von
Histogrammen unterscheiden.
